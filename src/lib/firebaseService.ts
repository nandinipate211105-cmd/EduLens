...
export function subscribeStudents(callback: (students: Student[]) => void) {
  ...
  return onSnapshot(q, snap => {
    ...
    console.warn('subscribeStudents error:', err.message);
    callback([]);
  });
}
...
export function subscribeAttendanceRange(dates: string[], callback: (records: AttendanceRecord[]) => void) {
  // Firestore "in" supports up to 10 values; good for 7-day charts.
  const safeDates = (dates || []).filter(Boolean).slice(0, 10);
  if (safeDates.length === 0) {
    callback([]);
    return () => {};
  }
  const q = query(collection(db, 'attendance'), where('date', 'in', safeDates));
  return onSnapshot(q, snap => {
    callback(snap.docs.map(d => d.data() as AttendanceRecord));
  }, (err) => {
    console.warn('subscribeAttendanceRange error:', err.message);
    callback([]);
  });
}

export async function fetchAttendanceRange(dates: string[]): Promise<AttendanceRecord[]> {
  const safeDates = (dates || []).filter(Boolean).slice(0, 10);
