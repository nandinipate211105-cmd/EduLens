      ...
      <div className="rounded-2xl border border-white/10 bg-[#0f1a2e] p-6">
        ...
        {sorted.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-white/5 p-6 text-center">
            <p className="font-syne text-sm font-bold text-white">No students found</p>
            <p className="mt-1 text-xs text-[#64748b]">Use “Seed Demo Data” or add a student in Learning Gaps.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {sorted.map((s, i) => {
              ...
              return (
                <div key={s.id || `${s.name}-${i}`} className="rounded-xl border border-white/10 p-4">
