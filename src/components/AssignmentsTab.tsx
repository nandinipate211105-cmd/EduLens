...
import { Assignment, UserProfile, Student } from '../types';
import { subscribeAssignments, updateAssignmentStatus, subscribeStudents } from '../lib/firebaseService';
...
export default function AssignmentsTab({ user }: Props) {
  ...
  // For student role: we need to find their Firestore student doc ID
  const [studentFirestoreId, setStudentFirestoreId] = useState<string | null>(null);

  const isTeacher = user?.role === 'teacher' || user?.role === 'admin';

  // For students, find their Firestore doc ID by matching display name
  useEffect(() => {
    ...
    const unsub = subscribeStudents(students => {
      ...
      setStudentFirestoreId(match?.id || null);
    });
    return () => unsub();
  }, [user, isTeacher]);

  // Subscribe to assignments once we have the right ID
  useEffect(() => {
    if (!user) return;
    // Teachers use their uid. Students need the Firestore doc ID.
    // If student ID not found yet, skip (wait for it).
    if (!isTeacher && studentFirestoreId === null) {
      // Give a brief grace period then show empty state
      const t = setTimeout(() => setLoading(false), 3000);
      return () => clearTimeout(t);
    }
    const idToUse = isTeacher ? user.uid : (studentFirestoreId || user.uid);
    const unsub = subscribeAssignments(idToUse, user.role, data => {
      setAssignments(data);
      setLoading(false);
    });
    return () => unsub();
  }, [user, isTeacher, studentFirestoreId]);
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-white/10 bg-[#0f1a2e] p-8 text-center">
          ...
          <p className="font-instrument text-sm text-[#64748b] mt-1">
            {!isTeacher && !studentFirestoreId
              ? 'Your student profile was not found. Ask your teacher to seed demo data first.'
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      {a.grade !== undefined && a.grade !== null && (
                        <span className="rounded-full bg-[#34d399]/10 px-2 py-0.5 text-[10px] font-bold text-[#34d399]">{a.grade}%</span>
                      )}
                    </div>
                    ...
                    {(a as any).studentName && isTeacher && (
                      <p className="mt-1 font-instrument text-xs text-[#64748b]">👤 {(a as any).studentName}</p>
                    )}
                  </div>
                  {a.status === 'pending' && !isTeacher && (
