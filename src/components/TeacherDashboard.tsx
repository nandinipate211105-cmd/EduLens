...
export default function TeacherDashboard({ user }: Props) {
  ...
  const subjectGapChartData = useMemo(() => {
    const roster = students.length === 0 ? getDemoClassStudents(user.uid) : students;
    const enriched = roster.map(s => fillMissingStudentFields(s));
    return CLASS_SUBJECT_GAPS.map(g => ({
      subject: g.subject,
      struggling: enriched.filter(s =>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-2xl border border-white/10 bg-[#0f1a2e] p-6">
          ...
          <p className="mb-2 text-xs text-[#64748b]">
            {students.length === 0
              ? 'Demo class (12 profiles) — seed or add students in Firebase for your real class.'
              : 'Live: students with “Needs Work” topics per subject.'}
          </p>
          <div className="h-[260px] touch-manipulation">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={subjectGapChartData} layout="vertical" margin={{ left: 4, right: 12 }}>
                ...
                <Bar dataKey="struggling" radius={[0, 6, 6, 0]} isAnimationActive={false}>
                  {subjectGapChartData.map((_, i) => (
