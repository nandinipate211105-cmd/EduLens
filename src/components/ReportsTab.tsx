...
import { subscribeStudents, fetchAttendanceRange } from '../lib/firebaseService';
...
export default function ReportsTab({ user }: Props) {
  ...
  const avgScore = useMemo(
    () => students.length ? Math.round(students.reduce((a, s) => a + (s.score || 0), 0) / students.length) : 0,
    [students]
  );
  ...
  const exportStudentsCSV = () => {
    const rows = [
      ['name', 'grade', 'score', 'attendance', 'assignments', 'riskLevel', 'riskScore'],
      ...students.map(s => [s.name, s.grade, s.score, s.attendance, s.assignments, s.riskLevel, s.riskScore ?? ''].map(String)),
    ];
    ...
    a.href = url;
    a.download = `EduLens_Students_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  ...
  const exportRiskCSV = () => {
    ...
    const rows = [
      ['name', 'grade', 'riskLevel', 'riskScore', 'score', 'attendance', 'assignments'],
      ...sorted.map(s => [s.name, s.grade, s.riskLevel, s.riskScore ?? '', s.score, s.attendance, s.assignments].map(String)),
    ];
    const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g, '""')}"`).join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `EduLens_Risk_List_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };

  const exportAttendanceCSV = async () => {
    // Export last 7 days summary per student
    const last7 = Array.from({ length: 7 }, (_, i) => {
      const d = new Date(); d.setDate(d.getDate() - (6 - i));
      return d.toISOString().split('T')[0];
    });
    const recs = await fetchAttendanceRange(last7);
    const rows: string[][] = [['studentName', 'studentId', ...last7]];
    students.forEach(s => {
      const sid = s.id || '';
      const row = [s.name, sid];
      last7.forEach(date => {
        const r = recs.find(x => x.studentId === sid && x.date === date);
        row.push(r ? (r.present ? 'Present' : 'Absent') : 'Unmarked');
      });
      rows.push(row);
    });
    const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g, '""')}"`).join(',')).join('\n');
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `EduLens_Attendance_Last7_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-2xl border border-white/10 bg-[#0f1a2e] p-5">
          ...
          <p className="mt-2 font-syne text-3xl font-extrabold text-white">{students.length}</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-[#0f1a2e] p-5">
      ...
      <div className="rounded-2xl border border-white/10 bg-[#0f1a2e] p-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button onClick={() => { const high = students.filter(s => s.riskLevel === 'HIGH'); alert(`High Risk Students: ${high.map(s => s.name).join(', ')}`); }}
          ...
          <button onClick={exportStudentsCSV}
            className="flex items-center gap-2 rounded-xl bg-white/5 border border-white/10 px-4 py-2.5 text-sm font-bold text-white hover:bg-white/10 transition-all">
            <Download size={16} /> Export Students CSV
          </button>
          <button onClick={exportRiskCSV}
            className="flex items-center gap-2 rounded-xl bg-white/5 border border-white/10 px-4 py-2.5 text-sm font-bold text-white hover:bg-white/10 transition-all">
            <Download size={16} /> Export Risk CSV
          </button>
          <button onClick={exportAttendanceCSV}
            className="flex items-center gap-2 rounded-xl bg-white/5 border border-white/10 px-4 py-2.5 text-sm font-bold text-white hover:bg-white/10 transition-all">
            <Download size={16} /> Export Attendance (Last 7) CSV
          </button>
        </div>
      </div>
