...
export default function StudentDashboard({ user }: Props) {
  ...
  useEffect(() => {
    const unsub = subscribeStudents(data => {
      setStudents(data);
      const roster = data.length > 0 ? data : getDemoClassStudents(user.uid);
      const found = pickStudentRecord(user, roster);
      setMyData(found ? fillMissingStudentFields(found) : null);
      setLoading(false);
    });
    return () => unsub();
  }, [user.displayName, user.email, user.uid]);

  const usingDemo = students.length === 0;

  const performanceCurve = useMemo(() => getStudentPerformanceCurve(myData), [myData]);
      ...
      {!demo ? (
        <div className="rounded-2xl border border-white/10 bg-[#0f1a2e] p-8 text-center">
          ...
          <p className="font-instrument text-sm text-[#64748b] mt-1">
            Ask your teacher to add your record. If the class list is empty, demo data will load after refresh.
          </p>
        </div>
      ) : (
        <>
          {usingDemo && (
            <div className="rounded-xl border border-[#fbbf24]/30 bg-[#fbbf24]/10 px-4 py-3 font-instrument text-sm text-[#fbbf24]">
              Demo student data — Firebase has no class roster yet. Showing a sample profile until your teacher adds records.
            </div>
          )}
          ...
          <div className={`grid grid-cols-1 gap-6 ${radarData.length > 0 ? 'lg:grid-cols-2' : ''}`}>
            <div className="rounded-2xl border border-white/10 bg-[#0f1a2e] p-6">
              <h3 className="mb-4 font-syne text-lg font-bold text-white">Performance curve (score %)</h3>
              <div className="h-[220px] touch-manipulation">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={performanceCurve.length ? performanceCurve : [{ label: '—', score: demo.score ?? 0, classAvg: 68 }]} margin={{ left: 8, right: 8, top: 8, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="score" stroke="#38bdf8" strokeWidth={2.5} dot={{ fill: '#38bdf8', r: 3 }} name="You" isAnimationActive={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
                ...
                <div className="h-[220px] touch-manipulation">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart data={radarData}>
                      ...
                      <Radar name="Score" dataKey="score" stroke="#38bdf8" fill="#38bdf8" fillOpacity={0.2} strokeWidth={2} isAnimationActive={false} />
                    </RadarChart>
                  </ResponsiveContainer>
                </div>
              </div>
