...
export default function MyPerformance({ user }: Props) {
  ...
  const roster = useMemo(() => {
    ...
    return getDemoClassStudents(user?.uid || 'local-demo');
  }, [students, user?.uid]);

  const demo = useMemo(() => {
    if (!roster.length) return null;
    const raw = user ? pickStudentRecord(user, roster) : roster[0];
    return raw ? fillMissingStudentFields(raw) : null;
  }, [roster, user]);

  const performanceCurve = useMemo(() => getStudentPerformanceCurve(demo), [demo]);
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="rounded-2xl border border-white/10 bg-[#0f1a2e] p-6">
            <h3 className="mb-4 font-syne text-lg font-bold text-white">Performance curve (score % — same as Learning Gaps)</h3>
            ...
            <div className="h-[240px] touch-manipulation">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={subjectData}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: '#0f1a2e', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', color: '#fff' }} />
                  <Bar dataKey="myScore" fill="#38bdf8" fillOpacity={0.8} radius={[4, 4, 0, 0]} name="My Score" isAnimationActive={false} />
                  <Bar dataKey="classAvg" fill="#818cf8" fillOpacity={0.5} radius={[4, 4, 0, 0]} name="Class Avg" isAnimationActive={false} />
                  <Legend />
                </BarChart>
              </ResponsiveContainer>
            </div>
